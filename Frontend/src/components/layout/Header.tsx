...
export const Header: React.FC<HeaderProps> = ({
  ...
  const titleMap: Record<ActiveTab, { title: string; subtitle: string }> = {
    analytics: {
      ...
      subtitle: 'Sotuvlar va buyurtmalar bo\'yicha real vaqtdagi ko\'rsatkichlar',
    },
    products: {
      title: 'Mahsulotlar Boshqaruvi',
      subtitle: 'Katalog, narxlar va zaxira holati boshqaruvi',
    },
    orders: {
      title: 'Jonli Buyurtmalar',
      subtitle: 'Mijozlardan tushgan yangi buyurtmalarni tasdiqlash va boshqarish',
    },
    'custom-cakes': {
      ...
      <div className="flex items-center gap-2 sm:gap-4 shrink-0">
        ...
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-medium">
          ...
          <span className="truncate max-w-[140px]">
            {telegramUser ? `TG: ${telegramUser.first_name}` : 'Admin'}
          </span>
        </div>
