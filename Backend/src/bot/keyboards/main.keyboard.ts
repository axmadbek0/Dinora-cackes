...
export function getMainKeyboard(lang: SupportedLanguage = 'uz') {
  if (lang === 'ru') {
    return new Keyboard()
      .text('🎂 Каталог десертов')
      .text('✨ Свой дизайн торта')
      .row()
      .text('🛒 Корзина')
      .text('🚚 Доставка и заказы')
      .row()
  ...
  if (lang === 'uz-Cyrl') {
    return new Keyboard()
      .text('🎂 Тайёр Тортлар')
      .text('✨ Ўзим хоҳлаганимдек Торт Build')
      .row()
      .text('🛒 Саватча')
      .text('🚚 Етказиб бериш & Кузатиш')
      .row()
      .text('📞 Алоқа & Маълумот')
  ...
  return new Keyboard()
    .text('🎂 Tayyor Tortlar')
    .text('✨ O\'zim xohlaganimdek Tort Build')
    .row()
    .text('🛒 Savatcha')
    .text('🚚 Yetkazib berish & Buyurtmalar')
    .row()
    .text('📞 Aloqa & Ma\'lumot')
