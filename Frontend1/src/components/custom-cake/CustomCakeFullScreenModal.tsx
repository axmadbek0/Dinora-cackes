...
export const CustomCakeFullScreenModal: React.FC<CustomCakeFullScreenModalProps> = ({
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (remainingSlots <= 0) {
      alert("Faqat 2 ta rasm yuklash mumkin!");
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (images.length !== 2) {
      alert("Iltimos, aynan 2 ta namuna rasm yuklang!");
      return;
    }
    if (!description.trim()) {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-0 lg:p-6 bg-black/60 backdrop-blur-sm overflow-hidden select-none">
          ...
          <motion.div
            ...
            className="relative z-10 bg-[#FAF6F0] w-full h-[100dvh] lg:h-auto lg:max-h-[92vh] lg:max-w-4xl lg:rounded-3xl lg:border lg:border-[#2B1810]/10 flex flex-col justify-between overflow-hidden shadow-2xl"
          >
            {/* Top Header Bar */}
            <div className="sticky top-0 z-10 px-4 sm:px-8 py-3.5 bg-white/95 backdrop-blur-md border-b border-[#2B1810]/10 flex items-center justify-between shadow-sm shrink-0">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-base sm:text-lg md:text-xl font-bold font-serif text-[#2B1810]">
                    ✨ O'zim xohlaganimdek
                  </h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto w-full space-y-6">
              {submittedOrder ? (
                ...
                <div className="bg-white rounded-3xl p-6 sm:p-12 border border-[#2B1810]/10 shadow-sm text-center my-auto space-y-5">
                  ...
                  <button
                    type="button"
                    onClick={handleClose}
                    className="min-h-[44px] bg-[#2B1810] text-[#FAF6F0] px-8 py-3.5 rounded-2xl font-bold text-sm shadow-md hover:bg-[#3D2318] active:scale-95 transition-all border border-[#CBB279] touch-manipulation"
                  >
                    Tushunarli, Rahmat!
                  </button>
                </div>
                ...
                /* Custom Builder Form */
                <form onSubmit={handleSubmit} className="bg-white p-5 sm:p-8 rounded-3xl border border-[#2B1810]/10 shadow-sm space-y-6">
                  
                  {/* Form Main Title Header */}
                  <div className="space-y-2 border-b border-[#2B1810]/10 pb-4">
                    ...
                    <h2 className="text-xl sm:text-2xl lg:text-3xl font-serif font-bold text-[#2B1810] leading-tight">
                      Qanday tort hohlaysiz, buni 2ta rasm va izohlab bering
                    </h2>
                            ...
                            {img ? (
                              <>
                                ...
                                <button
                                  ...
                                  className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full shadow-md hover:bg-red-600 active:scale-95 transition-all touch-manipulation"
                                  title="O'chirish"
                                >
                              ...
                              <label className="cursor-pointer w-full h-full flex flex-col items-center justify-center p-4 touch-manipulation">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="sm:col-span-2 space-y-1.5">
                      ...
                      <textarea
                        ...
                        className="w-full p-3.5 bg-[#FAF6F0] border border-[#2B1810]/10 rounded-2xl text-xs sm:text-sm text-[#2B1810] focus:outline-none focus:ring-2 focus:ring-[#D65B78]/40 resize-none touch-manipulation"
                        required
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        className="w-full min-h-[44px] p-3.5 bg-[#FAF6F0] border border-[#2B1810]/10 rounded-2xl text-xs sm:text-sm text-[#2B1810] font-bold focus:outline-none touch-manipulation"
                      >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <button
                        ...
                        className={`min-h-[44px] p-3.5 rounded-2xl text-xs sm:text-sm font-bold border flex items-center justify-center space-x-2 transition-all touch-manipulation ${
                    ...
                    {deliveryType === 'DELIVERY' && (
                      <input
                        ...
                        className="w-full p-3.5 bg-[#FAF6F0] border border-[#2B1810]/10 rounded-2xl text-xs sm:text-sm text-[#2B1810] focus:outline-none focus:ring-2 focus:ring-[#D65B78]/40 touch-manipulation"
                      />
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5 bg-[#F8E7EA] p-4 rounded-2xl border border-[#D65B78]/30">
                    ...
                    <input
                      ...
                      maxLength={17}
                      className="w-full p-3.5 bg-white border border-[#D65B78]/40 rounded-xl text-xs sm:text-sm font-bold text-[#2B1810] focus:outline-none focus:ring-2 focus:ring-[#D65B78] touch-manipulation"
                      required
                    />
                    <p className="text-[11px] text-[#6B5B52]">
                  ...
                  <button
                    ...
                    className="w-full min-h-[48px] bg-[#2B1810] hover:bg-[#3D2318] text-[#FAF6F0] py-4 rounded-2xl font-bold text-sm sm:text-base shadow-dinora-gold active:scale-98 transition-all flex items-center justify-center space-x-2 border-2 border-[#D4AF37] touch-manipulation safe-area-bottom"
                  >
